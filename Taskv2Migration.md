...
- **No system GDAL in the image.** The legacy Dockerfile injected C-level GDAL/PROJ from `ghcr.io/lambgeo/lambda-gdal` (a holdover from when `rasterio`/`pyproj` wheels didn't bundle their own libraries). That stage is dropped: the pinned `rasterio` (1.5.x) and `pyproj` (3.7.x) wheels are `manylinux_2_28` and ship bundled GDAL/PROJ/GEOS, and the Lambda base (Amazon Linux 2023, glibc 2.34) satisfies that tag. `task.py` uses `rasterio` only — no `osgeo` bindings — so nothing needs a system `libgdal`. Keeping the lambgeo layer was also actively harmful: it pinned GDAL 3.8 and set `GDAL_DATA`/`PROJ_LIB` at those libs, conflicting with the version rasterio's wheel carries. Removing it also unblocks the build (the lambgeo Python 3.12 tag it depended on doesn't exist). Build deps trimmed to `git` (for the pystac git dependency). The compose handler string was also corrected to `sentinel_2_l2a_to_stac.task.lambda_handler` (was `task.handler`) to match the Dockerfile `CMD`.
